var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
function clearCurrencyTable() {
    var $table = document.querySelector('#currency-table');
    $table.replaceChildren();
}
function createExchanteRatesTable(baseCurrency, currencies) {
    clearCurrencyTable();
    var supportedCodes = currencies.supportedCodes;
    var currencyRate = baseCurrency.conversionRates;
    var $table = document.querySelector('#currency-table');
    for (var i = 0; i < supportedCodes.length; i += 1) {
        var $tableCurrencyCode = document.createElement('td');
        $tableCurrencyCode.setAttribute('class', 'table-currency-code');
        $tableCurrencyCode.textContent = "".concat(supportedCodes[i][0]);
        var $tableCurrencyName = document.createElement('td');
        $tableCurrencyName.setAttribute('class', 'table-currency-name');
        $tableCurrencyName.textContent = "".concat(supportedCodes[i][1]);
        var $tableCurrencyRate = document.createElement('td');
        $tableCurrencyRate.setAttribute('class', 'table-currency-rate');
        $tableCurrencyRate.textContent = "".concat(currencyRate[supportedCodes[i][0]]);
        var $tableRow = document.createElement('tr');
        $tableRow.setAttribute('class', 'table');
        $tableRow.appendChild($tableCurrencyCode);
        $tableRow.appendChild($tableCurrencyName);
        $tableRow.appendChild($tableCurrencyRate);
        $table.appendChild($tableRow);
    }
}
function exchangeRatesDate(baseCurrency) {
    var currentDate = baseCurrency.lastUpdate;
    var currentCurrency = document.querySelector('#base-currency-title');
    currentCurrency.textContent = baseCurrency.baseCode;
    var $currentDate = document.querySelector('#current-date');
    $currentDate.textContent = currentDate.slice(0, 16);
}
function showCurrencyEquivalency(baseCurrency, conversionRate, targetCurrency) {
    var $equivalency = document.querySelector('#equivalency');
    var $baseEquivalency = document.querySelector('#base-equivalency');
    var $targetEquivalency = document.querySelector('#target-equivalency');
    $baseEquivalency.textContent = "1 ".concat(baseCurrency.baseCode);
    $targetEquivalency.textContent = "".concat(conversionRate, " ").concat(targetCurrency);
    if ($equivalency.classList.contains('d-none')) {
        $equivalency.classList.remove('d-none');
    }
}
function calculateExchange(baseCurrency, amount, targetCurrency) {
    var $results = document.querySelector('#target-amount');
    var conversionRate = baseCurrency.conversionRates[targetCurrency];
    $results.value = (conversionRate * amount).toString();
    showCurrencyEquivalency(baseCurrency, conversionRate, targetCurrency);
}
function createBaseCurrencyOptions(currencies) {
    var $baseCurrency = document.querySelector('#base-currency');
    var supportedCodes = currencies.supportedCodes;
    for (var i = 0; i < supportedCodes.length; i += 1) {
        var currency = document.createElement('option');
        currency.setAttribute('value', supportedCodes[i][0]);
        currency.textContent = "".concat(supportedCodes[i][0], " - ").concat(supportedCodes[i][1]);
        $baseCurrency.appendChild(currency);
    }
}
function createTargetCurrencyOptions(currencies) {
    var $targetCurrency = document.querySelector('#target-currency');
    var supportedCodes = currencies.supportedCodes;
    for (var i = 0; i < supportedCodes.length; i += 1) {
        var currency = document.createElement('option');
        currency.setAttribute('value', supportedCodes[i][0]);
        currency.textContent = "".concat(supportedCodes[i][0], " - ").concat(supportedCodes[i][1]);
        $targetCurrency.appendChild(currency);
    }
}
function handleExchangeCalculation(baseCurrency, amount) {
    var $targetCurrency = document.querySelector('#target-currency');
    var targetCurrency = $targetCurrency.value;
    var currentAmount = Number(amount);
    if (targetCurrency !== '0' && targetCurrency !== undefined)
        calculateExchange(baseCurrency, currentAmount, targetCurrency);
}
function getExchange(baseCurrency, callBackFunction) {
    var $amount = document.querySelector('#base-amount');
    $amount.oninput = function () {
        callBackFunction(baseCurrency, $amount.value);
    };
}
function generateCurrencyOptions(currencies) {
    createBaseCurrencyOptions(currencies);
    createTargetCurrencyOptions(currencies);
}
export default function handleExchange(currencies, callBackFunction) {
    var _this = this;
    generateCurrencyOptions(currencies);
    var $baseCurrency = document.querySelector('#base-currency');
    $baseCurrency.oninput = function () { return __awaiter(_this, void 0, void 0, function () {
        var baseCurrency;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, callBackFunction($baseCurrency.value)];
                case 1:
                    baseCurrency = _a.sent();
                    createExchanteRatesTable(baseCurrency, currencies);
                    exchangeRatesDate(baseCurrency);
                    getExchange(baseCurrency, handleExchangeCalculation);
                    return [2];
            }
        });
    }); };
}
//# sourceMappingURL=currencyExchange.js.map