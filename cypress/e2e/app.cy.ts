beforeEach(() => {
  cy.intercept("GET", "**/codes", { fixture: "currencies.json" }).as(
    "GetCurrencies"
  );
  cy.intercept("GET", "**/latest/*", { fixture: "USD.json" }).as(
    "USDExchangeRates"
  );
});
...
describe("Visit Exchange Rates App", () => {
  it("Homepage displays", () => {
    ...
    cy.get('[data-cy="title"]').should("have.text", "Exchange Rates App");
  });
});
...
describe("Exchange Rate Service Displays", () => {
  ...
  it("Currency Exchange between Base and Target based on Amount", () => {
    ...
    cy.get('[data-cy="base-currency-amount"]').type("1");
    cy.get('[data-cy="target-currency-amount"]').should(
      "have.value",
      "141.4841"
    );
  });
  ...
  it("Equivalency is properly shown on exchange", () => {
    cy.currenciesSelection();
    cy.get('[data-cy="equivalency"]').should(
      "contain.text",
      "1 USD is equivalent to 141.4841 ARS."
    );
  });
});
